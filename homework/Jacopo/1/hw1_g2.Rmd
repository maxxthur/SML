...
The problem basically says that we have $N$ groups, and for each we have $J$ realizations $y_{ij}$. Therefore, we will indicate from now on the total number of observations as $K = N \times J$. The least squares problem is the following:
...
sum_k^K  \(y_k - \overline{y_k} \)^2 \; + \; 2 \; sum_i^N \(y_k - \overline{y_k} \) \(\overline{y_k} - f_{\theta}(x) \) \;
